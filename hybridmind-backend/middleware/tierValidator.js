...
const { hasWorkflowAccess, validateWorkflowMode } = require('../config/workflowModes');
...
/**
 * Middleware: Validate workflow mode access
 */
async function validateWorkflowAccess(req, res, next) {
  try {
    ...
    // Validate workflow mode against tier
    const validation = validateWorkflowMode(tier, workflowMode, modelCount);
    
    if (!validation.valid) {
      return res.status(403).json({
        error: 'Workflow mode not available',
