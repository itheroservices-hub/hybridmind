 ...
 * Enhanced with new planning module for improved plan generation
 */
class Planner {
  constructor() {
    this.defaultModel = 'meta-llama/llama-3.3-70b-instruct'; // Use OpenRouter's Llama model for planning
    this.useNewPlanningModule = true; // Flag to enable new planning system
  }
   ...
   * @param {string} params.taskType - Task type (refactor/feature/debug/optimize/general)
   * @returns {Promise<Object>} Plan with steps
   */
  async createPlan({ goal, code, model = this.defaultModel, autonomous = true, taskType = 'general' }) {
    // Use new planning module if enabled
    if (this.useNewPlanningModule) {
      logger.info('🆕 Using enhanced planning module');
      ...
      try {
        ...
        if (result.success) {
          logger.info(`✅ Plan generated: ${result.plan.steps.length} steps`);
          return result.plan;
        } else {
          logger.warn('New planning failed, falling back to legacy planner');
          // Fall through to legacy planner
        }
      } catch (error) {
        logger.error(`New planning module error: ${error.message}`);
        // Fall through to legacy planner
      }
    }
